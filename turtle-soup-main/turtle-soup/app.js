  ...
  app.innerHTML = `
    ...
    <section class="host-grid">
      <article class="panel answer-panel">
        <h2>汤底</h2>
        <button class="hold-answer-button" type="button" data-action="toggle-answer" aria-pressed="false" aria-describedby="hold-help">点击查看汤底</button>
        <p class="helper-text">再次点击隐藏</p>
        <p id="hold-help" class="sr-only">点击按钮显示或隐藏汤底。</p>
...
function showAnswer() {
  ...
  const button = app.querySelector("[data-action='toggle-answer']");
  if (!soup || !box) return;
  state.isAnswerVisible = true;
  window.TurtleAnalytics?.track("answer_click", "shown");
  box.textContent = soup.answer;
  box.classList.add("is-visible");
  box.setAttribute("aria-hidden", "false");
  button?.setAttribute("aria-pressed", "true");
  if (button) button.textContent = "点击隐藏汤底";
}
...
function stopAnswerHold() {
  ...
  if (button) button.textContent = "点击查看汤底";
}

function toggleAnswer() {
  if (state.isAnswerVisible) stopAnswerHold();
  else showAnswer();
}
...
function handleAppClick(event) {
  ...
  if (action === "toggle-answer") toggleAnswer();
